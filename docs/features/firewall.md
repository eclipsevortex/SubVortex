...
The file will be automatically cleaned by removing any packets that belong to a request for which a SYNC packet is received before the defined time window. This time window is determined as the maximum between the time window specified in the rules (e.g., DoS, DDoS, etc.) and the default time window, which is 1 minute.
...
To install the UI, please refer to the [UI Guide](../../ui/README.md)
