...
async def challenge_data(self):
    ...
    bt.logging.debug(f"[Challenge] Step starting")
    ...
    for idx, (uid, (verified, response)) in enumerate(
        ...
        if not verified:
            if idx not in remove_reward_idxs:
                remove_reward_idxs.append(idx)
            bt.logging.warning(f"[Challenge][{uid}] The challenge could not be verified")
            continue

        bt.logging.success(f"[Challenge][{uid}] The challenge is verified")
        ...
        bt.logging.info(f"[Challenge][{uid}] Process time {process_time}")


    # Display step time
    forward_time = time.time() - start_time
    bt.logging.debug(f"[Challenge] Step finished in {forward_time:.2f}s")

    return remove_reward_idxs
