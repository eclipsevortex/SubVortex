...
async def compute_reliability_score(uid, success, database, hotkey: str):
    stats_key = f"stats:{hotkey}"

    # Step 1: Retrieve statistics
    # The current challenge is not saved yet in the database so we need to add it manually
    challenge_successes = int(
        await database.hget(stats_key, "challenge_successes") or 0
    ) + (1 if success else 0)
    challenge_attempts = (
        int(await database.hget(stats_key, "challenge_attempts") or 0) + 1
    )
    bt.logging.trace(
