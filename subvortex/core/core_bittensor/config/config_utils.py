...
def update_config(settings, config, prefix="SUBVORTEX"):
    ...
    for full_key in _gather_config_keys(config):
        ...
        if env_key in os.environ:
            ...
            value = _infer_type_from_config(config, parts, raw_value)
            print(f"{env_key} {raw_value} {parts}")
            _set_real_config_attr(config, parts, value)
...
def _infer_type_from_config(config, parts, raw_value):
    current = config
    for part in parts[:-1]:
