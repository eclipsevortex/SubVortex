...
async def create_challenge(settings: ccs.Settings, node_type: str, **kargs) -> tuple:
    subtensor = kargs.get("subtensor")

    if node_type == "lite":
        return await _create_lite_challenge(settings=settings, subtensor=subtensor)
    else:
...
async def _create_lite_challenge(
    settings: ccs.Settings, subtensor: btcas.AsyncSubtensor
) -> tuple:
    try:
        ...
        return (params, block_hash, value)
