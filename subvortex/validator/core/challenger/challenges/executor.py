...
async def execute_challenge(
    ...
    challengers_nodes = list(
        chain.from_iterable([nodes[x.hotkey] for x in challengers])
    )
    ...
    challenge = await create_challenge(
        config=config, node_type=node_type_selected, subtensor=subtensor
    )
    ...
    btul.logging.debug(f"Executing executed", prefix=settings.logging_name)
        ...
        if challenge_result is None:
            challenge_result = challenges_result[hotkey] = (
                ...
                ccm.ChallengeResult.create_default(
                    chain=chain, type=type, is_reliable=True,
                )
            )
