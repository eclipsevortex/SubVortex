...
async def test_sync_miners_on_new_miners(monkeypatch):
    ...
    result_miners = await sync_miners(
        Settings(),
        db,
        neurons,
        miners,
    ...
    assert result_miners[0].moving_score == 0
    ...
    assert len(result_miners[0].availability_attempts) == 0
    assert len(result_miners[0].availability_successes) == 0
    assert len(result_miners[0].reliability_attempts) == 0
    assert len(result_miners[0].reliability_successes) == 0
    assert len(result_miners[0].latency_times) == 0
    assert len(result_miners[0].performance_attempts) == 0
    assert len(result_miners[0].performance_successes) == 0
    assert len(result_miners[0].performance_boost) == 0
...
async def test_sync_miners_multiple_new_miners(monkeypatch):
    ...
    for i in range(2):
        ...
        assert result_miners[i].moving_score == 0
        ...
        assert len(result_miners[i].availability_attempts) == 0
        assert len(result_miners[i].availability_successes) == 0
        assert len(result_miners[i].reliability_attempts) == 0
        assert len(result_miners[i].reliability_successes) == 0
        assert len(result_miners[i].latency_times) == 0
        assert len(result_miners[i].performance_attempts) == 0
        assert len(result_miners[i].performance_successes) == 0
        assert len(result_miners[i].performance_boost) == 0
...
async def test_sync_miners_on_hotkey_change(monkeypatch):
    ...
    miner.moving_score = 0.8
    miner.score = 0.83
    miner.availability_score = 0.45
    miner.reliability_score = 0.65
    miner.latency_score = 0.47
    miner.distribution_score = 0.46
    miner.availability_attempts = [1, 1, 1]
    miner.availability_successes = [1, 0, 1]
    miner.reliability_attempts = [1, 1, 1]
    miner.reliability_successes = [1, 0, 1]
    miner.latency_times = [0.12, 0.13, 0.14]
    miner.performance_attempts = [1, 1, 1]
    miner.performance_successes = [1, 0, 1]
    miner.performance_boost = [0.12, 0.13, 0.14]
    miners = [miner]
    validator = fake_neuron(999, country="US")
...
async def test_sync_miners_on_ip_change_with_same_country(monkeypatch):
    ...
    assert result_miners[0].hotkey == neuron.hotkey
    assert result_miners[0].country == neuron.country
    assert result_miners[0].moving_score == 0.8
    assert result_miners[0].score == 0
    assert result_miners[0].availability_score == 0
    assert result_miners[0].reliability_score == 0
    assert result_miners[0].latency_score == 0
...
async def test_sync_miners_skips_unchanged_miners():
    ...
    db.remove_miner.assert_not_called()
    assert result_miners[0].uid == neuron.uid
    assert result_miners[0].rank == miner.rank
    ...
    assert result_miners[0].hotkey == neuron.hotkey
    assert result_miners[0].country == neuron.country
    assert result_miners[0].moving_score == miner.moving_score
    ...
    assert result_miners[0].availability_attempts == miner.availability_attempts
    assert result_miners[0].availability_successes == miner.availability_successes
    assert result_miners[0].reliability_attempts == miner.reliability_attempts
    assert result_miners[0].reliability_successes == miner.reliability_successes
    assert result_miners[0].latency_times == miner.latency_times
    assert result_miners[0].performance_attempts == miner.performance_attempts
    assert result_miners[0].performance_successes == miner.performance_successes
    assert result_miners[0].performance_boost == miner.performance_boost
