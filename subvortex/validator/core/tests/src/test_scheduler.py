...
import subvortex.validator.core.challenger.settings as svcce
import subvortex.validator.core.challenger.scheduler as svccs

from subvortex.validator.core.tests.mock.neuron import create_validator
...
async def test_get_schedules_when_the_number_of_validators_is_less_than_the_number_of_country_should_compute_the_right_schedule(
    ...
    assert len(validators) == len(schedules)
    ...
    for schedule in schedules.values():
        ...
        for i in range(len(schedule)):
            # # Check no collision in terms of country across all validators
    ...
    for i in range(len(countries)):
        ...
        assert len(validators) == len(scheduled_country)
...
async def test_get_schedules_when_the_number_of_validators_is_greater_than_the_number_of_country_should_compute_the_right_schedule(
    ...
    # Assert
    # Check not all the validators have a schedule
    assert len(validators) > len(schedules)
